...
#define AVB_AVTP_AAF_HDR_GET_TSV(hdr)		(hd->h.f.b1.ts_valid & 0x01)
#define AVB_AVTP_AAF_HDR_SET_TSV(hdr, val)	(hdr->h.f.b1.ts_valid = (hdr->h.f.b1.ts_valid | (val & 0x01)))
...
#define AVB_AVTPDU_CTRL_HDR_GET_MSGTYPE(hdr)		((hd->h.f.b1.msg_type & 0x0f))
#define AVB_AVTPDU_CTRL_HDR_SET_MSGTYPE(hdr, val)	(hdr->h.f.b1.msg_type = (hdr->h.f.b1.msg_type | ((val) & 0x0f)))
#define AVB_AVTPDU_CTRL_HDR_GET_VALIDTIME(hdr)		((hd->h.f.b2.valid_time & 0xf8) >> 3)
#define AVB_AVTPDU_CTRL_HDR_SET_VALIDTIME(hdr, val)	(hdr->h.f.b2.valid_time = (hdr->h.f.b2.valid_time | ((val << 3) & 0xf8)))
#define AVB_AVTPDU_CTRL_HDR_GET_DATALEN(hdr)		((((hdr->h.f.b2.data_len & 0x0007) << 8) & 0xff00) | (hdr->h.f.data_len & 0x00ff))
#define AVB_AVTPDU_CTRL_HDR_SET_DATALEN(hdr, val)	((hdr->h.f.b2.data_len = (hdr->h.f.b2.data_len | ((val >> 8) & 0x0007))), (hdr->h.f.data_len = (val & 0x00ff)))
...
struct socketdata {
	...
	u8 tx_buf[AVB_MAX_ETH_FRAME_SIZE];
	u8 rx_buf[AVB_MAX_ETH_FRAME_SIZE];
};

struct avtp_stream_info {
	int sr;
	int st;
	int seq_no;
	int socket_count;
...
struct avbcard {
	...
	struct avtp_stream_info tx;
	struct avtp_stream_info rx;
};
