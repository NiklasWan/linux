...
void avb_avtp_aaf_header_init(u8* buf, struct snd_pcm_substream *substream, struct snd_pcm_hw_params *hw_params)
{
	struct avbcard *avbcard = snd_pcm_substream_chip(substream);
	struct ethhdr *eh = (struct ethhdr *)&buf[0];
	struct avt_pdu_aaf_pcm_hdr* hdr = (struct avt_pdu_aaf_pcm_hdr*)&buf[sizeof(struct ethhdr)];

	printk(KERN_INFO "avb_avtp_aaf_header_init");
