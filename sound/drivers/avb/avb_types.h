...
struct stream_descp {
	...
	u8 bkd_up_talker_entity_id[8];
	u16 bkd_up_talker_unique_id;
	u16 avb_if_idx;
	u32 buf_size;
...
static bool avb_socket_init(struct socketdata* sd, int rx_timeout);

static int avb_get_avtp_aaf_nsr(int sampleRate);
static int avb_get_avtp_aaf_format(int rtformat);
static void avb_avtp_aaf_header_init(u8* buf, struct snd_pcm_substream *substream, struct snd_pcm_hw_params *hw_params);
