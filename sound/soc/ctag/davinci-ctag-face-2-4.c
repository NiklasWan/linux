...
static int snd_davinci_audiocard_probe(struct platform_device *pdev)
{
	...
	*/
	ret = of_property_read_u32(np, "ti,audio-codec-bit-delay-dac", &drvdata->ad1938_bit_delay_dac);
	if (ret < 0){
		dev_warn(&pdev->dev, "No bit delay defined for AD1938 dac. Using bit delay of 1.\n");
		drvdata->ad1938_bit_delay_dac = 1;
	}
	ret = of_property_read_u32(np, "ti,audio-codec-bit-delay-adc", &drvdata->ad1938_bit_delay_adc);
	if (ret < 0){
		dev_warn(&pdev->dev, "ti,No bit delay defined for AD1938 adc. Using bit delay of 1.\n");
		drvdata->ad1938_bit_delay_adc = 1;
	}
	ret = of_property_read_u32(np, "ti,mcasp-controller-bit-delay-tx", &drvdata->mcasp_bit_delay_tx);
	if (ret < 0){
		dev_warn(&pdev->dev, "No bit delay defined for McASP tx. Using bit delay of 1.\n");
		drvdata->mcasp_bit_delay_tx = 1;
	}
	ret = of_property_read_u32(np, "ti,mcasp-controller-bit-delay-rx", &drvdata->mcasp_bit_delay_rx);
	if (ret < 0){
